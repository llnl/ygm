...
class parquet_parser {
 ...
 private:
  ...
  template <typename Function>
    requires std::invocable<Function, const std::vector<parquet_type_variant> &>
  void read_parquet_files(Function fn,
  ...
  std::vector<parquet_type_variant> typed_batch_read(
    ...
    while (typed_reader->HasNext()) {
      ...
      const auto rows_read =
          typed_reader->ReadBatch(1, nullptr, nullptr, &value, &values_read);

      // Ensure only one value is read
      if (rows_read != 1) {
        std::cerr << "Error reading reading more than one rows. " << rows_read
                  << " rows read." << std::endl;
        std::abort();
      }
      ...
      if (values_read == 0) {
        ...
        if constexpr (std::is_same_v<value_type, parquet::ByteArray>) {
          ...
          read_values.emplace_back(std::string(static_cast<std::string_view>(value)));
        } else {
